/**
 * Debug script to test CNN model integration
 */
const { spawn } = require('child_process');
const path = require('path');
const fs = require('fs');

async function debugCNN() {
  console.log('🔍 Debugging CNN Model Integration');
  console.log('=' * 50);
  
  try {
    // Create a test image
    console.log('1. Creating test image...');
    const testImagePath = 'debug_test.jpg';
    
    const jpegData = Buffer.from([
      
    ]);
    
    fs.writeFileSync(testImagePath, jpegData);
    console.log(`✅ Created test image: ${testImagePath}`);
    
    // Test Python path
    console.log('\n2. Testing Python path...');
    const pythonPath = path.join(__dirname, '../cnn_model/app.py');
    console.log('📁 Python script path:', pythonPath);
    console.log('📁 File exists:', fs.existsSync(pythonPath));
    
    // Test Python execution
    console.log('\n3. Testing Python execution...');
    
    const pythonProcess = spawn('python', [
      pythonPath,
      '--image', testImagePath
    ], {
      shell: true
    });
    
    let result = '';
    let error = '';
    
    pythonProcess.stdout.on('data', (data) => {
      result += data.toString();
      console.log('📤 Python stdout:', data.toString());
    });
    
    pythonProcess.stderr.on('data', (data) => {
      error += data.toString();
      console.log('❌ Python stderr:', data.toString());
    });
    
    pythonProcess.on('close', (code) => {
      console.log(`\n🐍 Python process exited with code: ${code}`);
      console.log('📤 Final stdout:', result);
      console.log('❌ Final stderr:', error);
      
      if (code === 0) {
        try {
          const prediction = JSON.parse(result);
          console.log('✅ Parsed prediction:', prediction);
        } catch (parseError) {
          console.log('❌ Failed to parse output:', parseError.message);
        }
      } else {
        console.log('❌ Python process failed');
      }
      
      // Clean up
      try {
        fs.unlinkSync(testImagePath);
        console.log('✅ Cleaned up test image');
      } catch (cleanupError) {
        console.log('⚠️  Could not clean up test image:', cleanupError.message);
      }
    });
    
  } catch (error) {
    console.log('❌ Debug failed:', error.message);
  }
}

// Run the debug
debugCNN();
